...
def tmdb_config():
    from kodipopcorntime.caching import shelf, del_cache_file
    try:
        with shelf("com.imdb.conf", DEFAULT_TTL) as conf:
            if not conf:
                from kodipopcorntime.utils import url_get_json
                conf.update(url_get_json("%s/configuration" % BASE_URL, params={"api_key": API_KEY}, headers=HEADERS, with_immunicity=False))
                if not conf:
                    raise
            return dict(conf)
    except:
    ...
    try:
        with shelf("com.imdb.%s" % imdb_id, DEFAULT_TTL) as movie:
            if not movie:
                ...
                movie.update(url_get_json("%s/movie/%s" % (BASE_URL, imdb_id), params={"api_key": API_KEY, "append_to_response": "credits", "language": "en", "include_image_language": "en,null"}, headers=HEADERS, with_immunicity=False))
                if not sys_lang == 'en':
                    movie.update(url_get_json("%s/movie/%s" % (BASE_URL, imdb_id), params={"api_key": API_KEY, "append_to_response": "credits", "language": sys_lang, "include_image_language": "%s" % sys_lang}, headers=HEADERS, with_immunicity=False) or {})
                if not movie:
                    raise
        return dict(movie)
    except:
        ...
        del_cache_file("com.imdb.%s" % imdb_id)

def search(query):
    from kodipopcorntime.utils import url_get_json
    return url_get_json("%s/search/movie" % BASE_URL, params={"api_key": API_KEY, "query": query, "language": "en"}, headers=HEADERS, with_immunicity=False)
